...
#### /api-token-auth/
Used for retrieving tokens. 
Accepts: POST.

### User endpoints
#### /users/
Accesses all users. Only available to super users.
...
#### /signup/
Registers user and returns token for authentication.
Accepts: GET, POST.

### Store endpoints
#### /stores/
Accesses all stores associated with user.
Accepts: GET, POST.
#### /stores/{id}/
Accesses a specific store and allows editing.
Accepts: GET, PUT, PATCH, DELETE.
#### /days-list/
Lists stores and filters them by days that they are open.
Accepts: GET.
Note: It appears to accept PATCH, but such requests are given a 405 response.
#### /days-detail/{id}/
Used to edit which days a store is open.
Accepts: GET, PATCH.
#### /hours-list/
Lists stores and filters them by their hours of operation.
Accepts: GET.
Note: It appears to accept PATCHa and PUT, but such requests are given a 405 response.
#### /hours-detail/{id}/
Used to edit hours of operation of a store.
Accepts: GET, PATCH, PUT.
#### /managers-list/
Lists stores and filters them by their managers.
Accepts: GET.
Note: It appears to accept PATCH and PUT, but such requests are given a 405 response.
#### /managers-detail/{id}/
Used to edit the managers of a store.
Accepts: GET, PATCH, PUT.
