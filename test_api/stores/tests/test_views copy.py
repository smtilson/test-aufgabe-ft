...
class StoreDaysViewTests(BaseTestCase):
    def setUp(self):
        super().setUp()
        self.url = f"stores/{self.store1.id}/days"  # reverse("store-days", kwargs={"pk": self.store1.id})

    def test_get_store_days_valid_id(self):
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)
        days = response.data["days_of_operation"]
        expected_days = [
            "Montag",
            "Dienstag",
            "Mittwoch",
        ...
        self.assertEqual(days, str(expected_days))
        self.assertIn("message", str(response.data))
        self.assertIn("Modify the days of operation by", response.data["message"])

    def test_get_store_days_without_id(self):
        url = "stores/days/"  # reverse("store-days")
    ...
    def test_get_store_days_invalid_id(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(
            response.data["detail"], "Store not found. Please provide a valid store ID."
        )

    @skip("skipping temporarily")
    def test_get_store_days_non_numeric_id(self):
        url = "/stores/days/invalid/"
        response = self.client.get(url)
        print(response.data)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
